...
class _ChecklistInterventionScreenState extends State<ChecklistInterventionScreen> {
  ...
  Future<void> _updateItem(ItemChecklistModel item, String nouveauStatut, String? gravite, String? remarque) async {
    try {
      ...
      _loadChecklist(); // Recharger pour afficher l'état mis à jour
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Erreur: $e'), backgroundColor: Colors.red));
      }
  ...
  Future<void> _cloturerIntervention() async {
    ...
    try {
      // 1. Clôturer la checklist
      await _checklistService.cloturerChecklist(_checklist!.id, {
        'bilanIntervention': _bilanController.text.trim(),
        'estMaintenance': true, // À adapter selon la logique réelle
        'estDepannage': false,
        'estTravaux': false,
      });

      // 2. (Optionnel) Clôturer le Bon de Travail lui-même si ton backend le demande ainsi
      // await _btService.terminerIntervention(widget.bonTravailId, {'diagnostic': 'Voir checklist', 'actionRealisee': _bilanController.text.trim()});
      ...
      if (mounted) {
        ...
        Navigator.pop(context); // Retour à la liste des interventions
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Erreur: $e'), backgroundColor: Colors.red));
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      Expanded(
                                        child: _buildStatusChip(item, 'CONFORME', Colors.green),
                                      ),
                                      const SizedBox(width: 8),
                                      Expanded(
                                        child: _buildStatusChip(item, 'ANOMALIE_DETECTEE', Colors.red),
                                      ),
                                    ],
                                  ),
                                  // Afficher les champs supplémentaires si anomalie
                        ...
                        children: [
                          TextFormField(
                            ...
                            decoration: const InputDecoration(
                              labelText: 'Bilan global de l\'intervention *',
