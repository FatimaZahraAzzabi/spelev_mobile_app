...
class _EditProfilScreenState extends State<EditProfilScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_newPhotoFile != null) {
        await _service.modifierPhoto(_newPhotoFile!.path);
      }
      ...
      await _service.modifierProfil({
        ...
        'nomEntreprise': _entrepriseController.text.trim().isEmpty
            ? null
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      if (_newPhotoFile != null)
                        const Text('Nouvelle photo sélectionnée ✓',
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  const Text('Email (non modifiable)',
  ...
  // ═══════════════════════════════════════════════════════════
  // Widget Avatar intelligent :
  // - Nouvelle photo locale (FileImage) → priorité
  // - Sinon photo existante via PROXY BACKEND + JWT
  // - Sinon initiale
  // ═══════════════════════════════════════════════════════════
  Widget _buildAvatar(bool aPhoto) {
    final initiale = (widget.profil.prenom ?? '').isNotEmpty
        ? widget.profil.prenom![0].toUpperCase()
        : '?';

    // 1. Nouvelle photo sélectionnée (locale) → FileImage
    if (_newPhotoFile != null) {
      return CircleAvatar(
        radius: 60,
    ...
    // 2. Pas de photo du tout → initiale
    if (!aPhoto) {
      return CircleAvatar(
        radius: 60,
    ...
    // 3. Photo existante → PROXY BACKEND avec JWT
    if (_token == null) {
    ...
    debugPrint(' Edit avatar via proxy: $proxyUrl');
    ...
    return CircleAvatar(
      ...
      onBackgroundImageError: (error, _) {
        debugPrint(' Erreur chargement photo edit: $error');
      },
      child: null,
    );
  }
